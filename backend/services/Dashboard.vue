...
import { emitVoiceStop } from '../utils/voiceSession'
...
const reminderRepeatMs = ref(120000)

const loadReminderSettings = async () => {
  try {
    const { data } = await getReminderSettings()
    reminderRepeatMs.value = Math.max(60, (data.repeat_minutes || 2) * 60 * 1000)
  } catch {
    reminderRepeatMs.value = 120000
  }
}

const restartVoiceNudgeTimer = () => {
  window.clearInterval(voiceNudgeTimer)
  voiceNudgeTimer = null
  if (!activeReminder.value) return
  voiceNudgeTimer = window.setInterval(() => {
    void nudgeReminder().catch(() => {})
  }, reminderRepeatMs.value)
}

watch(activeReminder, () => {
  restartVoiceNudgeTimer()
})
