...
const statusHint = computed(() => {
  ...
  if (phase.value === 'capturing') return '正在对焦拍照，大约需要 10～20 秒，请拿稳药盒...'
...
const capturePhotoWithTimeout = async (ms = 45000, epoch = sessionEpoch) => {
  ...
  const timer = window.setTimeout(() => captureAbortController.abort(), ms)
  try {
...
const capturePhotoOnly = async ({
  ...
  if (handoff) {
    await pauseReminderTts(90).catch(() => {})
  }
...
const runInitialTurnCore = async (presetQuestion = '', epoch = sessionEpoch, { fastHandoff = false } = {}) => {
  ...
  if (fromHandoff) {
    ...
    const answered = await answerQuestion(preset, '拍照问药', epoch)
...
const runFollowupTurnCore = async (epoch = sessionEpoch) => {
  ...
  let heard = typed
  if (!heard) {
    const listened = await listenQuestion(FOLLOWUP_LISTEN_HINT, epoch)
    if (!isLiveSession(epoch)) return { kind: 'abort' }
