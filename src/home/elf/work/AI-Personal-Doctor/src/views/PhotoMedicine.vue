        ...
        <div ref="messageListRef" class="message-list">
          <div v-if="!messages.length && isReminderVerifySession" class="welcome-card">
            ...
            <p>
              ...
              若拍错了或不是该吃的药，拿<strong>正确的药盒</strong>后说「<strong>重新拍</strong>」再拍一次。
              说「<strong>退出</strong>」返回首页。
            </p>
          </div>
...
import {
  ...
  getVoiceSessionBusy,
  isSessionExitPhrase,
  prepareDemoForCapture,
  releaseVoiceEngineForCapture,
  stopAllTts,
...
const CAPTURE_DONE_SPEECH = '拍好了，请说出您想问的问题。说切换问诊可以转到问诊，说退出返回首页'
...
const FOLLOWUP_LISTEN_HINT = '请继续提问。换一盒说换一个，说切换问诊可以转到问诊，说退出返回首页'
const RECAPTURE_LISTEN_HINT = '请说出您想问的问题。说切换问诊可以转到问诊，说退出返回首页'
const REMINDER_CAPTURE_SPEECH = '请拿今天该吃的药盒，对准摄像头，两秒后拍照'
const REMINDER_RECAPTURE_SPEECH = '这盒可能不对，请拿今天提醒该吃的药，对准摄像头'
const REMINDER_FOLLOWUP_LISTEN = '若药盒不对，请拿正确的药盒说重新拍，或点下方重新拍照核对；说退出返回首页'
const REMINDER_POST_VERIFY_SPEECH =
  '已确认药品正确。您可以继续问怎么吃、注意事项或副作用，说退出返回首页。'
const REMINDER_POST_VERIFY_LISTEN = '请提问，例如怎么吃、注意什么；说退出返回首页'
const REMINDER_WRONG_SPEECH = '若这盒不对，请拿正确的药盒，说重新拍再拍一次'
const REMINDER_RECAPTURE_PHRASES = [
  '重新拍',
  '再拍',
  '不对',
  '不是这个',
...
const wantsReminderRecapture = (text) => {
  const normalized = (text || '').trim()
  if (!normalized) return false
  return REMINDER_RECAPTURE_PHRASES.some((phrase) => normalized.includes(phrase))
}

const isWrongMedicineAnswer = (text) => {
  const normalized = (text || '').trim()
  if (!normalized) return false
  if (/是该吃|可以服用|请按时|没问题|药品正确|是对的|确认无误/.test(normalized)) {
    return false
  }
  if (/^不是[,，、]|不是该吃|不是这个药|不是今天|包装上是|请先别吃|别吃|不能服|不对/.test(normalized)) {
    return true
  }
...
const sessionStatusLabel = computed(() => {
  ...
  if (isReminderVerifySession.value) {
    ...
    if (hasCapturedPhoto.value) return '用药确认 · 不对可说重新拍 · 说退出返回'
...
const btnLabel = computed(() => {
  ...
  if (photoSessionActive.value && hasCapturedPhoto.value) {
    if (reminderNeedsRecapture.value && isReminderVerifySession.value) {
      return hasTypedQuestion.value ? '换盒后发送' : '重新拍照核对'
    }
...
const statusHint = computed(() => {
  if (isReminderVerifySession.value) {
    if (phase.value === 'capturing') return '正在拍照核对今天该吃的药，请拿稳药盒...'
    if (phase.value === 'answering') return 'AI 正在核对是不是提醒里的药...'
    if (phase.value === 'listening') return '请说重新拍或退出...'
  }
...
const capturePhotoWithTimeout = async (ms = 120000, epoch = sessionEpoch) => {
  const timeoutMs = Math.max(ms, 90000)
...
const capturePhotoOnly = async ({
  ...
  if (!handoff) {
    ...
    scrollToBottom()
    try {
      await speakText(REMINDER_CAPTURE_SPEECH, true)
    } catch {
      /* 提示语失败不阻断拍照 */
    }
    if (!isLiveSession(epoch)) {
  ...
  phase.value = 'capturing'
  scrollToBottom()
  await sleep(handoff ? 600 : PHOTO_HOLD_MS)
  refreshPhotoHandoff()
  if (!isLiveSession(epoch)) {
    const err = new Error('问药已取消')
    err.name = 'SessionAbort'
  ...
  for (let attempt = 1; attempt <= 2; attempt += 1) {
    try {
      ...
      await sleep(400)
      refreshPhotoHandoff()
      const { data } = await capturePhotoWithTimeout(120000, epoch)
      ...
      if (attempt < 2 && busy) {
        ElMessage.warning('语音引擎正忙，5 秒后重试拍照…')
        await sleep(5000)
        continue
      }
      if (error?.code === 'ERR_CANCELED' || error?.name === 'CanceledError' || error?.name === 'AbortError') {
        throw new Error('拍照超时，请检查摄像头后重试')
      }
      throw error
    }
  }
...
const answerQuestion = async (questionText, label = '拍照问药', epoch = sessionEpoch) => {
  ...
  if (isReminderVerifySession.value && isWrongMedicineAnswer(assistantText)) {
...
const runInitialTurnCore = async (presetQuestion = '', epoch = sessionEpoch, { fastHandoff = false } = {}) => {
  ...
  if (fromHandoff) {
    ...
    await waitForTtsIdle(15000)
    const answered = await answerQuestion(preset, isReminderVerifySession.value ? '用药确认' : '拍照问药', epoch)
...
const runReminderRecapture = async (epoch = sessionEpoch) => {
  keepReminderSessionQuiet()
  await speakText(REMINDER_RECAPTURE_SPEECH, true)
  if (!isLiveSession(epoch)) return { kind: 'abort' }
  await capturePhotoOnly({
    handoff: true,
    skipDoneSpeech: true,
  ...
  const answered = await answerQuestion(verifyQ, '用药确认', epoch)
  ...
  if (answered.kind === 'verify_wrong') {
    try {
      await speakText(REMINDER_WRONG_SPEECH, true)
...
const runFollowupTurnCore = async (epoch = sessionEpoch) => {
  ...
  let heard = typed
  if (!heard) {
    const listenHint = isReminderPostVerifySession.value
      ? REMINDER_POST_VERIFY_LISTEN
      : isReminderVerifySession.value
        ? REMINDER_FOLLOWUP_LISTEN
        : FOLLOWUP_LISTEN_HINT
  ...
  if (
    isReminderVerifySession.value &&
    (action === 'recapture' || wantsReminderRecapture(heard) || reminderNeedsRecapture.value)
  ) {
    if (reminderNeedsRecapture.value && action === 'followup' && !wantsReminderRecapture(heard)) {
      try {
        await speakText('当前这盒不对，请先拿正确的药盒，我帮您重新拍', true)
        await waitForTtsIdle(12000)
      } catch {
        /* optional */
      }
    ...
    return runReminderRecapture(epoch)
  }
  if (action === 'recapture') {
    await speakText('好的，请把新药盒对准摄像头', true)
    if (!isLiveSession(epoch)) return { kind: 'abort' }
    await capturePhotoOnly()
    if (!isLiveSession(epoch)) return { kind: 'abort' }
  ...
  const answered = await answerQuestion(heard, '追问', epoch)
  return answered.kind === 'abort' ? { kind: 'abort' } : { kind: 'ok' }
}
...
const runAutoFollowupOnce = async (epoch = sessionEpoch) => {
  ...
  try {
    ...
    if (result?.kind === 'empty') {
      ...
      } else if (isReminderVerifySession.value) {
        ElMessage.info(
          reminderNeedsRecapture.value
            ? '没听到。请拿正确的药盒，说重新拍或点下方「重新拍照核对」'
            : '没听到。若药盒不对，请说重新拍；说退出返回首页'
        )
      } else {
  ...
  if (photoSessionActive.value && hasCapturedPhoto.value) {
    ...
    if (reminderNeedsRecapture.value && isReminderVerifySession.value) {
      ...
      try {
        ...
        } else if (result?.kind === 'verify_wrong') {
          scheduleAutoFollowup(sessionEpoch, 1200)
        }
      } catch (error) {
        ElMessage.error(formatError(error))
      } finally {
...
const handleWakePayload = async (payloadFromEvent = null) => {
  ...
  try {
    ...
    try {
      const result = await runInitialTurn(preset, epoch, { fastHandoff: Boolean(payload.fast_handoff) })
      if (!isLiveSession(epoch)) return
      if (result?.kind === 'busy') {
